<script>
  ...
  onMount(() => { if ($currentUser) ladeRechnungen(); });
  ...
  function oeffneBearbeitenModal(r) {
    bearbeitenRechnung = {
      ...
      einzelpreis: r.einzelpreis || 0,
      rabatt_pct: r.rabatt_pct || 0,
      order_id: r.order_id || ''
    };
  ...
  async function speichereBearbeitung() {
    if (!bearbeitenRechnung.kaeufer_name || !bearbeitenRechnung.artikel_name || !bearbeitenRechnung.einzelpreis) {
      showToast('Bitte alle Pflichtfelder ausfüllen.'); return;
    }
    bearbeitenLaeuft = true;
    try {
      const res = await apiCall('rechnung-bearbeiten', { user_id: $currentUser.id, ...bearbeitenRechnung });
      if (res.success) { showToast('Rechnung gespeichert ✓'); bearbeitenModal = false; await ladeRechnungen(); }
    } catch(e) { showToast('Fehler: ' + e.message); } finally { bearbeitenLaeuft = false; }
  }
  ...
  function statusBadge(r) {
    ...
    const map = {
      ...
      bearbeitet: { label: 'Bearbeitet', cls: 'badge-bearbeitet' }
    };
  ...
  const filterTabs = [
    ...
    { key: 'stornos', label: 'Stornos' }, { key: 'erstellt', label: 'Erstellt' },
...
<div class="page-container">
  ...
  <div class="kpi-grid">
    ...
    <div class="kpi-card kpi-card-wide"><div class="kpi-label">Umsatz Brutto (aktive)</div><div class="kpi-value kpi-blue">{fmt(kpiBrutto)} €</div></div>
  ...
  <div class="toolbar">
    ...
    <div class="toolbar-right">
      <input class="suche-input" type="text" placeholder="Suche Nr., Käufer, Artikel…" bind:value={suchbegriff} />
      ...
      <div class="vorschau-panel">
        ...
        <div class="vorschau-body">
          ...
          <div class="vorschau-section">
            ...
            {#if vorschauRechnung.kaeufer_email}<div class="vs-sub">{vorschauRechnung.kaeufer_email}</div>{/if}
            {#if vorschauRechnung.kaeufer_strasse}
              <div class="vs-sub">{vorschauRechnung.kaeufer_strasse}, {vorschauRechnung.kaeufer_plz} {vorschauRechnung.kaeufer_ort}, {vorschauRechnung.kaeufer_land}</div>
            {/if}
          </div>
          <div class="vorschau-section">
          ...
          <div class="betraege-box">
            <div class="betrag-zeile"><span>Netto</span><span>{fmt(vorschauRechnung.netto_betrag)} €</span></div>
            <div class="betrag-zeile"><span>MwSt. ({vorschauRechnung.steuersatz || 19} %)</span><span>{fmt(vorschauRechnung.steuer_betrag)} €</span></div>
            <div class="betrag-zeile betrag-brutto"><span>Brutto</span><span>{fmt(vorschauRechnung.brutto_betrag)} €</span></div>
          </div>
  ...
  {#if gefiltert.length > 0}
    <div class="footer-bar">
      <div class="footer-summen">
        ...
        {#if summeStorno > 0}<span class="storno-summe">Stornos: <strong>{fmt(summeStorno)} €</strong></span>{/if}
    ...
    <div class="modal-box" onclick={(e) => e.stopPropagation()}>
      ...
      <div class="modal-body">
        <div class="form-grid">
          ...
          <div class="form-group form-2col">
            <div class="form-group"><label>Land</label><input bind:value={neueRechnung.kaeufer_land} placeholder="DE" /></div>
            <div class="form-group"><label>eBay Bestellnr.</label><input bind:value={neueRechnung.order_id} placeholder="12-34567-89012" /></div>
          </div>
  ...
  <div class="modal-overlay" onclick={() => sendenModal = false}>
    <div class="modal-box modal-klein" onclick={(e) => e.stopPropagation()}>
      ...
      <div class="modal-body">
        ...
        <div class="form-group"><label>Empfänger-E-Mail</label><input bind:value={sendenEmail} type="email" placeholder="kaeufer@example.com" /></div>
      </div>
      <div class="modal-footer">
...
{#if stornoModal}
  <div class="modal-overlay" onclick={() => stornoModal = false}>
    <div class="modal-box modal-klein" onclick={(e) => e.stopPropagation()}>
      ...
      <div class="modal-body">
        <p class="modal-info">Möchtest du die Rechnung <strong>{stornoRechnung?.rechnung_nr}</strong> wirklich stornieren? Es wird automatisch eine Stornorechnung erstellt.</p>
      </div>
      <div class="modal-footer">
...
{#if bearbeitenModal && bearbeitenRechnung}
  <div class="modal-overlay" onclick={() => bearbeitenModal = false}>
    <div class="modal-box" onclick={(e) => e.stopPropagation()}>
      <div class="modal-hdr">
        <span class="modal-titel">✏️ Bearbeiten — {bearbeitenRechnung.rechnung_nr}</span>
        <button class="icon-btn" onclick={() => bearbeitenModal = false}>✕</button>
      </div>
      <div class="modal-body">
        <div class="form-grid">
          <div class="form-group form-2col">
            <div class="form-group"><label>Käufer *</label><input bind:value={bearbeitenRechnung.kaeufer_name} placeholder="Max Mustermann" /></div>
            <div class="form-group"><label>E-Mail</label><input type="email" bind:value={bearbeitenRechnung.kaeufer_email} placeholder="max@example.com" /></div>
          </div>
          <div class="form-group"><label>Straße</label><input bind:value={bearbeitenRechnung.kaeufer_strasse} placeholder="Musterstr. 1" /></div>
          <div class="form-group form-2col">
            <div class="form-group"><label>PLZ</label><input bind:value={bearbeitenRechnung.kaeufer_plz} placeholder="12345" /></div>
            <div class="form-group"><label>Ort</label><input bind:value={bearbeitenRechnung.kaeufer_ort} placeholder="Berlin" /></div>
          </div>
          <div class="form-group form-2col">
            <div class="form-group"><label>Land</label><input bind:value={bearbeitenRechnung.kaeufer_land} placeholder="DE" /></div>
            <div class="form-group"><label>Bestellnr.</label><input bind:value={bearbeitenRechnung.order_id} placeholder="12-34567-89012" /></div>
          </div>
          <div class="form-group"><label>Artikel *</label><input bind:value={bearbeitenRechnung.artikel_name} placeholder="Produktname" /></div>
          <div class="form-group form-2col">
            <div class="form-group"><label>Menge *</label><input type="number" min="1" bind:value={bearbeitenRechnung.menge} /></div>
            <div class="form-group"><label>Einzelpreis (Brutto) *</label><input type="number" step="0.01" bind:value={bearbeitenRechnung.einzelpreis} /></div>
          </div>
          <div class="form-group"><label>Rabatt (%)</label><input type="number" min="0" max="100" bind:value={bearbeitenRechnung.rabatt_pct} /></div>
        </div>
      </div>
      <div class="modal-footer">
        <button class="btn-ghost" onclick={() => bearbeitenModal = false}>Abbrechen</button>
        <button class="btn-primary" onclick={speichereBearbeitung} disabled={bearbeitenLaeuft}>{bearbeitenLaeuft ? 'Speichert…' : '💾 Speichern & PDF neu generieren'}</button>
      </div>
    </div>
  </div>
{/if}
...
<style>
  ...
  .icon-btn-edit:hover { color: #d97706; background: #fffbeb; }
  ...
  .badge-bearbeitet { background: #eff6ff; color: #2563eb; }
  ...
  :global([data-theme="dark"]) .badge-bearbeitet { background: rgba(37,99,235,.15);  color: #60a5fa; }
