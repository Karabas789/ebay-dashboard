<script>
  ...
  const blockDefaults = {
    ...
    divider:{type:'divider',style:'normal'},spacer:{type:'spacer',height:24},
  ...
  function toggleHtmlMode() {
    if (showHtmlMode) {
      if (richEditorBlockId) updateBlock(richEditorBlockId, richEditorField, htmlRawText);
      if (richEditorEl) richEditorEl.innerHTML = htmlRawText;
      showHtmlMode = false;
    } else {
      const block = blocks.find(b => b.id === richEditorBlockId);
      htmlRawText = block ? (block[richEditorField] || '') : '';
      showHtmlMode = true;
    }
  }
  ...
  function vorschauHtml() {
    return generateFullHtml().replace(/\{\{rechnung_nr\}\}/g,'RE-2026-00042').replace(/\{\{kaeufer_name\}\}/g,'Max Mustermann').replace(/\{\{datum\}\}/g,'22.04.2026').replace(/\{\{brutto_betrag\}\}/g,'49,99').replace(/\{\{firmenname\}\}/g,'Import & Produkte Vertrieb').replace(/\{\{bestellnummer\}\}/g,'12-34567-89012').replace(/\{\{artikelname\}\}/g,'Windows 11 Pro Produktschlüssel').replace(/\{\{variante\}\}/g,'Download-Version');
  }
  ...
  async function speichern() {
    ...
    try {
      ...
      await apiCall('email-config-speichern', {
        ...
        smtp_host: ec.smtp_host || '', smtp_port: ec.smtp_port || 587,
        smtp_user: ec.smtp_user || '', smtp_pass: '',
        smtp_secure: ec.smtp_secure || false,
        absender_name: ec.absender_name || '', absender_email: ec.absender_email || '',
    ...
    <div class="vb-builder">
      ...
      <div class="vb-props">
        {#if selectedBlock}
          ...
          {:else if selectedBlock.type==='text' || selectedBlock.type==='infobox'}
            ...
            <div class="vb-ps"><div class="vb-ps-t">Text bearbeiten</div>
              <div class="ed-toolbar">
                ...
                <span class="ed-sep"></span>
                <div class="ed-dd"><button class="ed-btn" onclick={()=>{showLinkDialog=!showLinkDialog;showColorPicker=false}} title="Link">🔗</button>
              ...
              {#if showHtmlMode}
                <textarea class="ed-html" rows="12" bind:value={htmlRawText} oninput={()=>{if(richEditorBlockId)updateBlock(richEditorBlockId,richEditorField,htmlRawText)}}></textarea>
              {:else}
                <!-- svelte-ignore a11y_click_events_have_key_events, a11y_no_static_element_interactions -->
          ...
          {/if}
        {:else}
          <div class="vb-props-empty"><div style="font-size:2rem;opacity:0.2;margin-bottom:10px">👈</div>Wähle einen Block</div>
        {/if}
      </div>
    </div>
  {/if}
