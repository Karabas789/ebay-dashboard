<script>
  ...
  async function ladeVorlage() {
    ...
    try {
      ...
      if (data && data.vorlage) vorlage = { ...vorlage, ...data.vorlage };
    } catch(e) {}
  }
  ...
  async function speichereVorlage() {
    vorlageLaeuft = true;
    try {
      await apiCall('vorlage-speichern', { user_id: $currentUser.id, vorlage });
  ...
  async function generiereVorschau() {
    ...
    aktiverTab = 'vorschau';
    try {
      const data = await apiCall('rechnung-vorschau', { user_id: $currentUser.id, vorlage, beispiel });
...
<div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:28px;padding-bottom:16px;border-bottom:${v.header_trennlinie ? v.header_trennlinie_staerke+'px solid '+v.header_trennlinie_farbe : 'none'};">
  ...
  <div style="text-align:right">${kontaktBlock}</div>
</div>
...
<div class="vb-container">
  ...
  <div class="vb-topbar">
    ...
    <div class="vb-topbar-actions">
      ...
      <button class="vb-btn-ghost" onclick={generiereVorschau} disabled={vorschauLaeuft}>
        {vorschauLaeuft ? '⏳ Lädt…' : '🖨 PDF-Vorschau'}
      </button>
      <button class="vb-btn-primary" onclick={speichereVorlage} disabled={vorlageLaeuft}>
    ...
    <div class="vb-settings" style="width:{settingsBreite}px; min-width:{settingsBreite}px;">
      ...
      <div class="sektion">
        <button class="sektion-hdr" onclick={() => toggleSektion('logo')}>
          <span>🖼 Logo</span>
          <span class="chevron" class:offen={offeneSektionen.has('logo')}>›</span>
        </button>
        ...
        <div class="sektion-body">
          {#if vorlage.logo_base64}
            <div class="logo-preview">
              ...
              <button class="vb-btn-danger vb-btn-sm" onclick={logoEntfernen}>Entfernen</button>
            </div>
          {:else}
      ...
      <div class="sektion">
        <button class="sektion-hdr" onclick={() => toggleSektion('farben')}>
          ...
          <span class="chevron" class:offen={offeneSektionen.has('farben')}>›</span>
        </button>
        ...
        <div class="sektion-body">
          <div class="form-row">
            <div class="form-group">
              ...
              <div class="color-row"><input type="color" bind:value={vorlage.akzentfarbe} class="color-input" /><input type="text" bind:value={vorlage.akzentfarbe} class="color-text" /></div>
            </div>
            <div class="form-group">
              <label>Tabellenfarbe</label>
              <div class="color-row"><input type="color" bind:value={vorlage.tabellenfarbe} class="color-input" /><input type="text" bind:value={vorlage.tabellenfarbe} class="color-text" /></div>
            </div>
          </div>
          <div class="form-row">
            <div class="form-group">
              <label>Schriftfarbe</label>
              <div class="color-row"><input type="color" bind:value={vorlage.schriftfarbe} class="color-input" /><input type="text" bind:value={vorlage.schriftfarbe} class="color-text" /></div>
            </div>
            <div class="form-group">
              <label>Hintergrund</label>
              <div class="color-row"><input type="color" bind:value={vorlage.hintergrundfarbe} class="color-input" /><input type="text" bind:value={vorlage.hintergrundfarbe} class="color-text" /></div>
            </div>
          </div>
          <div class="form-row">
            <div class="form-group">
          ...
          <div class="form-group">
            ...
            <div class="themen-grid">
              {#each [
                ...
                <button class="thema-btn" style="border-color:{thema.a}"
                  ...
                  <span class="thema-farbe" style="background:{thema.a}"></span>{thema.name}
                </button>
              {/each}
            </div>
          </div>
      ...
      <div class="sektion">
        <button class="sektion-hdr" onclick={() => toggleSektion('firma')}>
          ...
          <span class="chevron" class:offen={offeneSektionen.has('firma')}>›</span>
        </button>
      ...
      <div class="sektion">
        <button class="sektion-hdr" onclick={() => toggleSektion('header')}>
          <span>📄 Header</span>
          <span class="chevron" class:offen={offeneSektionen.has('header')}>›</span>
        </button>
        ...
        <div class="sektion-body">
          ...
          <div class="form-row">
            <div class="form-group">
              ...
              <div class="color-row"><input type="color" bind:value={vorlage.header_trennlinie_farbe} class="color-input" /><input type="text" bind:value={vorlage.header_trennlinie_farbe} class="color-text" /></div>
            </div>
            <div class="form-group">
          ...
          {/if}
          <div class="form-group"><label>Rechnungstitel</label><input bind:value={vorlage.titel_text} placeholder="Rechnung" /></div>
          <div class="form-group">
      ...
      <div class="sektion">
        <button class="sektion-hdr" onclick={() => toggleSektion('tabelle')}>
          ...
          <span class="chevron" class:offen={offeneSektionen.has('tabelle')}>›</span>
        </button>
      ...
      <div class="sektion">
        <button class="sektion-hdr" onclick={() => toggleSektion('footer')}>
          ...
          <span class="chevron" class:offen={offeneSektionen.has('footer')}>›</span>
        </button>
        {#if offeneSektionen.has('footer')}
        <div class="sektion-body">
          <div class="form-group"><label>Fußtext</label><textarea bind:value={vorlage.footer_text} rows="2" placeholder="Vielen Dank für Ihre Bestellung."></textarea></div>
          <div class="form-group">
            <label>Fußtext-Farbe</label>
            <div class="color-row"><input type="color" bind:value={vorlage.footer_farbe} class="color-input" /><input type="text" bind:value={vorlage.footer_farbe} class="color-text" /></div>
          </div>
          <div class="toggle-list">
          ...
          <div class="form-group" style="margin-top:12px"><label>Zahlungshinweis</label><input bind:value={vorlage.zahlungshinweis} placeholder="Bereits bezahlt über eBay" /></div>
          ...
          <div style="margin-top:14px;border-top:1px solid var(--border);padding-top:14px">
            ...
            {#if vorlage.kleinunternehmer}
              <div class="form-group" style="margin-top:8px"><label>Hinweistext</label><textarea bind:value={vorlage.kleinunternehmer_text} rows="2"></textarea></div>
            {/if}
            ...
            {#if vorlage.wasserzeichen}
              <div class="form-group" style="margin-top:8px"><label>Wasserzeichen-Text</label><input bind:value={vorlage.wasserzeichen_text} placeholder="ENTWURF" /></div>
            {/if}
          </div>
        </div>
        {/if}
    ...
    <div class="vb-preview" class:vb-preview-fullscreen={aktiverTab === 'vorschau'}>
      ...
      <div class="preview-frame-wrap">
        {#if aktiverTab === 'vorschau' && vorschauPDF}
          <iframe src="data:application/pdf;base64,{vorschauPDF}" title="PDF Vorschau" class="pdf-iframe"></iframe>
        {:else}
          <iframe srcdoc={vorschauHTML} title="HTML Vorschau" class="preview-iframe" sandbox="allow-same-origin"></iframe>
        {/if}
      </div>
    </div>
...
<style>
  /*
   * KEY FIX: vb-container nutzt jetzt 100% der Höhe des Parent-Wrappers
   * (page-wrapper-fill im Layout gibt flex:1 + overflow:hidden),
   * statt hartcodierten calc(100vh - 56px) der beim neuen Design falsch war.
   * margin:-28px -32px entfernt — das page-wrapper-fill hat padding:0.
   */
  .vb-container {
    display: flex;
  ...
  .tab-btn {
    ...
    border-radius: 6px; font-size: 0.8rem; color: var(--text2);
  ...
  /* Buttons — eigene Klassen um Konflikt mit globalen app.css zu vermeiden */
  .vb-btn-primary {
  ...
  .vb-btn-ghost {
    ...
    color: var(--text2); padding: 7px 12px; border-radius: 8px;
    font-size: 0.83rem; cursor: pointer; transition: all 0.15s; font-family: inherit;
  }
  ...
  .vb-btn-sm { padding: 5px 10px; font-size: 0.78rem; }
  ...
  .vb-body {
    ...
    flex: 1;
    overflow: hidden;
    min-height: 0; /* wichtig: verhindert dass flex-child den Parent sprengt */
  }

  /* Linke Settings-Spalte */
  .vb-settings {
    flex-shrink: 0;
    overflow-y: auto;
    overflow-x: hidden;
  ...
  /* Sektion-Gruppen */
  .gruppe-label {
    font-size: 0.67rem; font-weight: 800; color: var(--text3);
  ...
  .sektion-hdr {
    ...
    align-items: center; font-size: 0.84rem; font-weight: 700;
    color: var(--text); cursor: pointer; transition: background 0.15s;
    font-family: inherit;
  }
  .sektion-hdr:hover { background: var(--border); }
  .chevron { font-size: 1rem; color: var(--text2); display: inline-block; transform: rotate(90deg); transition: transform 0.2s; }
  ...
  .vb-divider::after {
    ...
    background: var(--text3); border-radius: 2px; opacity: 0.5;
  }
  .vb-divider:hover,
  .vb-divider.dragging { background: var(--primary); width: 5px; }
  .vb-divider:hover::after,
  .vb-divider.dragging::after { background: white; opacity: 1; }

  /* Vorschau-Panel */
  .vb-preview {
    flex: 1; display: flex; flex-direction: column;
    background: #cbd5e1; overflow: hidden; min-width: 0;
  }
  ...
  @media (max-width: 900px) {
    ...
    .vb-settings { width: 100% !important; min-width: unset !important; border-right: none; border-bottom: 1px solid var(--border); max-height: 45vh; }
